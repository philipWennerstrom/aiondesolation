...
/*            	
            	for (Npc npc: npcs)
            	{
                    ...
                    try {
                        ...
                        List <RouteStep> steps = template.getRouteSteps();
                        int stepSize = steps.size();
                        for (RouteStep step : steps) 
                        {
            	        	...
            	        	while (updatedZ > admin.getZ()) //Falling
            	        	{
            	        		...
            	        		while (retryCount < 6 && (oldAdminZ - admin.getZ()) > 20 || admin.getZ() <= (World.getInstance().getWorldMap(admin.getWorldId()).getDeathLevel() + 10))
            	        		{
            	        			if (retryCount == 5)
            	        			{
            	        				msg(admin, " Skipping step: ("+(npcCounter)+"/"+npcs.size()+") step [" + step.getRouteStep() + "/"+stepSize+"] - because there is an black hole ?!");            	        				
            	        			...
            	        			tempJumpHeight += 2;
            	        			msg(admin, "ReTry ("+retryCount+"/5) step: ("+(npcCounter)+"/"+npcs.size()+") step [" + step.getRouteStep() + "/"+stepSize+"] with new jumpheight: "+tempJumpHeight);
            	        			step.setZ(oldAdminZ + tempJumpHeight);
            	        			TeleportService2.teleportTo(admin, admin.getWorldId(), step.getX(), step.getY(), step.getZ());
            	        			admin.getController().stopProtectionActiveTask();
            	        			updatedZ = step.getZ() +3F;
            	        			oldAdminZ = step.getZ() +3F;
            	        			retryCount++;        	        			
            	        		}
                            ...
                            if (!corrections.containsKey(step.getRouteStep()))
                            	corrections.put(step.getRouteStep(), admin.getZ());
                            else
                            {
                            	corrections.remove(step.getRouteStep());
                            	corrections.put(step.getRouteStep(), admin.getZ());
                            }
                        }

                        msg(admin, "Saving corrections...");
                        ...
                        int lastStep = template.isReversed() ? (stepSize + 2) / 2
                                : stepSize;
                        for (int s = 0; s < lastStep; s++) {
                            RouteStep step = template.getRouteSteps().get(s);
                            RouteStep fixedStep = new RouteStep(step.getX(), step.getY(), corrections.get(step.getRouteStep()), step.getRestTime());
                            fixedStep.setRouteStep(step.getRouteStep());
                        ...
                        msg(admin, "Done with routeid: "+routeId);
                    } catch (Exception e) {
                    }
            	}
            	if (!canceled)
            	{
            		if (npcs.size() > 1)
            			data.saveData(admin.getWorldId()+"_"+World.getInstance().getWorldMap(admin.getWorldId()).getName());
            		else data.saveData("new_"+admin.getWorldId()+"_"+routeId);
            	}
*/
