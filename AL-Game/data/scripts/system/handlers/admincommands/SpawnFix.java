...
public class SpawnFix extends AdminCommand {
	...
	{		
		...
		for (SpawnGroup2 spawn : sd.getSpawnsByWorldId(worldId))
		{
			...
			else npc = DataManager.NPC_DATA.getNpcTemplate(spawn.getNpcId());
			...
			if (npc != null) template_sb.append("\t\t<!-- "+npc.getName()+" ||| "+npc.getDesc()+" -->\r\n");
			if (staticSpawn != null) template_sb.append("\t\t<!-- StaticObject: "+staticSpawn.getName()+" ||| "+staticSpawn.getNamedesc()+" -->\r\n");
			...
			for (SpawnTemplate st : spawn.getSpawnTemplates())
			{
					...
					if (st.getWalkerId() != null && !st.getWalkerId().isEmpty()) template_sb.append(" walker_id=\""+st.getWalkerId()+"\"");
					if (st.hasRandomWalk()) template_sb.append(" random_walk=\""+st.getRandomWalk()+"\"");
					if (st.getStaticId() > 0) template_sb.append(" static_id=\""+st.getStaticId()+"\"");
					if (st.getWalkerIndex() > 0) template_sb.append(" walker_index=\""+st.getWalkerIndex()+"\"");
					if (st.getFly() > 0) template_sb.append(" fly=\""+st.getFly()+"\"");					
