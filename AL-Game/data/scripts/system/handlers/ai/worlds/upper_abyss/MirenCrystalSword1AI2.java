 ...
 * Author Phantom_KNA
 */
@AIName("mirensword1") // Npc: 702843
public class MirenCrystalSword1AI2 extends NpcAI2 {
	...
	public boolean onDialogSelect(final Player player, int dialogId, int questId, int extendedRewardIndex) {
		switch (dialogId) {
			case 10000:
				if (player.getInventory().getFirstItemByItemId(182215913) != null) {
					return false;
				}
	...
	private boolean timerStartSpawnSpirit(Player player) {
		...
		this.taskSpawnSpirit = ThreadPoolManager.getInstance().schedule(new Runnable() {
			...
			public void run() {
				announceSpiritBoss();
				spawn(883662, 1895.8019f, 2450.727f, 2956.2307f, (byte) 66); // Spawn Berserk Guardian Miren
			};
		}, ((this.timeSpawnDelaySpirit * 60) * 1000)); 

		spawn(805489, 1895.8019f, 2450.727f, 2956.2307f, (byte) 66); // Spawn Spirit Flag
	...
	private void announceSpiritBoss() {
        World.getInstance().doOnAllPlayers(new Visitor<Player>() {
            ...
            public void visit(Player player) {
				PacketSendUtility.sendPacket(player, new SM_SYSTEM_MESSAGE(1403143));
            }
        });
    }
    ...
    protected void handleDialogStart(Player player) { 
		if (player.getInventory().decreaseByItemId(182215913, 1)) { //Miren Spirit Pendant ItemID: 182215913
